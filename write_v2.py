...
def _fetch_vol(ticker: str, period: str = "1y") -> ComparableTickerRow:
    ...
    for attempt, wait in enumerate(wait_times):
        try:
            ...
            is_rate_limit = any(x in err_msg for x in [
                "too many requests", "429", "rate limit", "throttl"
            ])
    ...
    return ComparableTickerRow(
        ...
        volatility=None, current_price=None,
...
def fetch_comparables(
    ...
    st.info(
        f"Fetching {total} ticker(s). "
        f"Requests are spaced 4-8s apart to avoid rate limiting."
    )
...
def show(case: dict):
    ...
    with tab1:
        ...
        g_col1.metric("Delta", f"{greeks['delta']:.4f}")
        g_col2.metric("Gamma", f"{greeks['gamma']:.4f}")
        g_col3.metric("Theta", f"{greeks['theta']:.4f}")
        g_col4.metric("Vega", f"{greeks['vega']:.4f}")
        g_col5.metric("Rho", f"{greeks['rho']:.4f}")
    ...
    with tab2:
        ...
        price_df = pd.DataFrame(
            ...
            columns=[f"Step {i}" for i in range(6)]
        )
    ...
    with tab3:
        ...
        ax2.axvline(float(np.mean(payoffs)), color="orange",
                    linestyle="--", label=f"Mean={np.mean(payoffs):.2f}")
    ...
    ticker_input = st.text_input(
        ...
        placeholder="Example: 7203.T, 9984.T, AAPL, MSFT"
    )
    ...
    if "comparable_results" in st.session_state:
        ...
        if vols:
            ...
            s_col1.metric("Mean Vol", f"{np.mean(vols):.1%}")
            s_col2.metric("Median Vol", f"{np.median(vols):.1%}")
            s_col3.metric("Min Vol", f"{np.min(vols):.1%}")
            s_col4.metric("Max Vol", f"{np.max(vols):.1%}")
"""
