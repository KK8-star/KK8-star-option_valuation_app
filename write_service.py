# write_service.py  ← プロジェクトルートに保存
code = """\
\"\"\"
ValuationService - オプション評価メインサービス
Black-Scholes (50%) + 二項モデル (30%) + モンテカルロ (20%)
\"\"\"
...
def _bs_greeks(S, K, T, r, sigma, q, is_call: bool) -> dict:
    ...
    delta = (np.exp(-q * T) * norm.cdf(d1)
             if is_call else -np.exp(-q * T) * norm.cdf(-d1))
    gamma = np.exp(-q * T) * nd1 / (S * sigma * sqT)
    vega  = S * np.exp(-q * T) * nd1 * sqT / 100
    if is_call:
        theta = (
            -S * np.exp(-q * T) * nd1 * sigma / (2 * sqT)
...
def _mc_price(S, K, T, r, sigma, q, is_call: bool,
              n_paths: int = 10_000, seed: int = 42) -> tuple[float, float]:
...
class VolatilityMeta:
    \"\"\"ボラティリティ計算メタデータ\"\"\"
    vol_method: str = "historical"   # "historical" | "manual"
    vol_period: str = "1y"           # "3m" | "6m" | "1y" | "2y" | "3y"
    vol_source: str = ""             # ティッカーまたはデータ説明
    vol_data:   str = ""             # JSON形式の価格データ(任意)


# ── ValuationService ──────────────────────────────────────────────────────────
class ValuationService:
    \"\"\"BS (50%) + 二項 (30%) + MC (20%) の加重平均\"\"\"
    ...
    def save(
        ...
        vol_meta: Optional[VolatilityMeta] = None,
    ) -> Optional[int]:
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                # 2. ValuationParameter (ボラティリティメタデータ含む)
...
# 検証
with open("src/services/valuation_service.py", "r", encoding="utf-8") as f:
    content = f.read()

checks = [
    ("VolatilityMeta クラス", "class VolatilityMeta"),
    ("vol_meta 引数", "vol_meta: Optional[VolatilityMeta]"),
    ("vol_method セット", "vol_method=meta.vol_method"),
    ("vol_period セット", "vol_period=meta.vol_period"),
    ("vol_source セット", "vol_source=meta.vol_source"),
    ("vol_data セット",   "vol_data=meta.vol_data"),
]
